...
//                            ((MapAdapter) pullrefreshlistview.getRefreshableView().getAdapter()).setItemDataSrc(new MapContent(list));
    ...
    private void loadConfig(List list) {
        ...
        MapConf m = MapConf.with(getBaseContext()).pair("match-level->level").pair("match-address->address").pair("match-start_time->date").pair("match-name->title").pair("match-cover_url->image").pair("video-pay_status->pay_status", "1:已支付;0:未支付", "Showpaystatus()").pair("video-sell_price->adapter_price").pair("video-title->video_name").pair("video-cover_url->adapter_bszb_img").source(R.layout.adapter_myvideo);
