...
public class V4MultLiveRightView extends RelativeLayout {
    ...
    public void addLiveView() {
        ...
        for (int i = 0; i < lives.size(); i++) {
            ...
            final String url = lives.get(i).getPreviewStreamPlayUrl();
            ...
            // int portraitHeight = dip2px(getContext(), 135);
            ...
            public void onStateResult(int event, Bundle bundle) {
                switch (event) {
                    ...
                    case PlayerEvent.PLAY_ERROR:
                        showNoSignalTip(rootView, mMultLivePlayHolder);
                        break;
...
//                    videoView.setVideoLayout(VideoViewSizeHelper.VIDEO_LAYOUT_STRETCH, 0);
    ...
    private void showNoSignalTip(ViewGroup rootView, MultLivePlayHolder mMultLivePlayHolder) {
        if (mVideoViews != null) {
            mVideoViews.get(mMultLivePlayHolder.location).onDestroy();
    ...
    public void setStreams(List<LiveInfo> lives) {
        this.lives = lives;
    }
    ...
    public int indexOfLiveInfo(String liveId) {
        for (LiveInfo info : lives) {
            if (TextUtils.equals(info.getLiveId(), liveId)) {
                return lives.indexOf(info);
            }
        }
        return -1;
    }
                    ...
                    if (holder.layout.equals(v)) {
                        if (holder.videoState && mCurrentIndex != holder.location) {
                            ...
                            if (mSwitchMultLiveCallback != null) {
                                mSwitchMultLiveCallback.switchMultLive(lives.get(mCurrentIndex).getLiveId());
                            }
                        } else {
