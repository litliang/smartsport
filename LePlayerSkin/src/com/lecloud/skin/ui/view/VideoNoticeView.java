...
//        });
    ...
    private void videoTips(String state, String errorReason, String tips, String confirm, String cancel) {
        showMsg(state, tips);
        if (mTips.getVisibility() == VISIBLE) {
    ...
    public void processMediaState(int event, Bundle data) {
        Log.e("wlc", "【processMediaState】event:" + event + ";data:" + data);
...
//            mEventCode = data.getString(PlayerParams.KEY_RESULT_ERROR_CODE, "");
            String err_subcode = data.getString(IStatsErrors.ERR_SUBCODE);
            mEventCode = data.getString(IStatsErrors.ERR_EC) + (TextUtils.isEmpty(err_subcode) ? "" : "-" + err_subcode);
            String err_ec = data.getString(IStatsErrors.ERR_EC);
            if (!TextUtils.isEmpty(err_ec) && err_ec.length() > 5) {
                String code = err_ec.substring(3, 5);
                if ("00".equals(code) || "02".equals(code)) {
                    ...
                    videoTips(mEventCode, getText(R.string.net_fail), getText(R.string.net_error), getText(R.string.replay), getText(R.string.submit_error_info));
                } else {
...
//            }
            ...
            if (data != null) {
                mEventCode = data.getString(IStatsErrors.ERR_EC) + "-00";
            }
            videoTips(mEventCode, getText(R.string.net_fail), getText(R.string.net_error), getText(R.string.replay), getText(R.string.submit_error_info));
        }
    }
    ...
    public void processPlayerState(int event, Bundle data) {
        Log.e("wlc", "【processPlayerState】event:" + event + ";data:" + data);
        if (!NetworkUtils.hasConnect(mContext)) {
            if (data != null) {
                mEventCode = data.getString(IStatsErrors.ERR_EC) + "-00";
            }
...
//                }
                if (data != null) {
                    if (NetworkUtils.hasConnect(mContext)) {
                        ...
                        mEventCode = data.getString(IStatsErrors.ERR_EC) + (TextUtils.isEmpty(err_subcode) ? "" : "-" + err_subcode);
                        if (!TextUtils.isEmpty(err_ec) && err_ec.length() > 5) {
                            String ec_code = err_ec.substring(3, 5);
                            if ("00".equals(ec_code) || "02".equals(ec_code)) {
                                String errMsg = getText(R.string.play_error);
                                videoTips(mEventCode, errMsg, getText(R.string.letv_notice_message), getText(R.string.replay), getText(R.string.submit_error_info));
                            } else if ("01".equals(ec_code)) {
                                videoTips(mEventCode, getText(R.string.net_fail), getText(R.string.net_error), getText(R.string.replay), getText(R.string.submit_error_info));
                            } else {
                                String errMsg = data.getString(PlayerParams.KEY_RESULT_ERROR_MSG);
                                videoTips(mEventCode, errMsg, getText(R.string.letv_notice_message), getText(R.string.replay), getText(R.string.submit_error_info));
